...
export function Launchpad({ apps, greeting, overview }: { apps: AppDef[]; greeting?: string; overview?: ReactNode }) {
  ...
  const group = (label: string, items: AppDef[]) =>
    items.length === 0 ? null : (
      <section className="mt-6">
        <p className="anno mb-3">{label}</p>
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-5">{items.map(tile)}</div>
      </section>
    );
  ...
  return (
    <div className="mx-auto max-w-6xl p-6">
      ...
      {overview}
      {group('Wertschöpfung', kette)}
      {group('Zentrale Funktionen', zentral)}
    </div>
  );
}
