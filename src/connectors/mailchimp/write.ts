...
export async function writeMailchimpSubscribers(data: CanonicalDataset): Promise<void> {
  if (data.subscribers.length === 0) {
    throw new Error('Mailchimp sync: 0 subscriber rows — aborting without deleting.');
  }
  const client = await pool.connect();
  try {
    await client.query('BEGIN');
    await client.query(`DELETE FROM subscribers WHERE source = 'mailchimp'`);
